...
pip install ete3
mkdir -m 775 /home/w3const/work-kosuge/etetoolkit
'''


def get_info_local(name_list):
    custom_path = "/home/w3const/work-kosuge/etetoolkit/ncbitaxonomy.sqlite"
    ncbi = NCBITaxa(dbfile=custom_path)
    # 辞書で返ってくる
    name_to_taxid = ncbi.get_name_translator(name_list)
    if len(name_to_taxid) == 0:
        return {name_list[0]: None}, {'Rank': None}
    else:
        taxid_to_rank = ncbi.get_rank(name_to_taxid[name_list[0]])
        return name_to_taxid, taxid_to_rank
    # 例
    # print(get_info_local(['Caulobacter soli', 'Staphylococcus aureus']))
...
def main():
    ...
    t = get_info_local([orgn])
    print(t)
    if t[0][orgn] is not None:
        print(f'taxid:{t[0][orgn][0]}')
    ...
    if t[0][orgn] is None:
        ...
        t = get_info_local([orgn])
        if t[0][orgn] is not None:
            print(f'Uppler lineage is {orgn}, taxid:{t[0][orgn][0]}')
        else:
            print(f'Uppler lineage is {orgn}, taxid:{t[0][orgn]}')

    # If you want to useEntrez seach instead of ete3
    ...
    #     orgn = tmp
    #     taxid, rank = get_taxid_and_rank(orgn)
    #     print(f"Upper lineage is {orgn}, taxid:{taxid}, rank:{rank}")
